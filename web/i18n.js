...
const I18N = {
  ...
  fr: {
    ...
    privacy_html: "<strong>Votre fichier ne quitte pas le navigateur.</strong> Cette page est statique, sans analytique et sans composant serveur propre. Au chargement, seuls <code>cdn.jsdelivr.net</code> (Pyodide), <code>pypi.org</code> et <code>files.pythonhosted.org</code> (bibliothèques) sont contactés ; l'hébergeur de la page voit la consultation comme n'importe quel site. Après le choix du fichier, plus aucune requête réseau n'a lieu - une Content-Security-Policy de la page interdit au navigateur de contacter toute autre adresse. La langue et le thème sont mémorisés localement par le navigateur.",
  ...
  it: {
    ...
    privacy_html: "<strong>Il file non lascia il browser.</strong> Questa pagina è statica, senza analytics e senza componente server propria. Al caricamento vengono contattati solo <code>cdn.jsdelivr.net</code> (Pyodide), <code>pypi.org</code> e <code>files.pythonhosted.org</code> (librerie); l'host della pagina vede la visita come qualsiasi sito web. Dopo la scelta del file non avviene più nessuna richiesta di rete: una Content-Security-Policy della pagina impedisce al browser di contattare qualsiasi altro indirizzo. Lingua e tema vengono ricordati localmente dal browser.",
